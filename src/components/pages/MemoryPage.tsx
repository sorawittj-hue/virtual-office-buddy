...
import {
  Brain, Clock, Tag, Trash2, Plus, Edit2, Check, X, Search,
...
const MEMORY_PROVIDERS = [
  { id: "local", label: "Local (localStorage)", icon: HardDrive, active: true, desc: "เก็บใน browser ของคุณ ไม่มีค่าใช้จ่าย" },
  { id: "mem0",  label: "Mem0",                 icon: Cloud,     active: false, desc: "Semantic memory cloud — mem0.ai" },
  { id: "honcho",label: "Honcho",               icon: Database,  active: false, desc: "User-level personalization layer" },
  { id: "local-embed", label: "Local Embeddings", icon: Cpu,     active: false, desc: "Vector search ด้วย embeddings ในเครื่อง" },
];
...
const DEFAULT_MEMORIES: MemoryEntry[] = [
  { id: "m1", key: "ภาษาที่ใช้", value: "ไทย (th)", category: "preference", createdAt: Date.now(), updatedAt: Date.now() },
  { id: "m2", key: "ช่องทางรับคำสั่ง", value: "Telegram Bot", category: "context", createdAt: Date.now(), updatedAt: Date.now() },
  { id: "m3", key: "ผู้ใช้งานหลัก", value: "เจ้านาย", category: "personal", createdAt: Date.now(), updatedAt: Date.now() },
];
  ...
  return (
    <div className="px-4 py-3 bg-muted/40 space-y-2">
      ...
      <div className="flex gap-2 justify-end">
        ...
        <button
          onClick={() => { if (key.trim() && value.trim()) onSave(key.trim(), value.trim(), category); }}
...
export function MemoryPage() {
  ...
  const addMemory = (key: string, value: string, category: MemoryCategory) => {
    const entry: MemoryEntry = { id: crypto.randomUUID(), key, value, category, createdAt: Date.now(), updatedAt: Date.now() };
  ...
  const updateMemory = (id: string, key: string, value: string, category: MemoryCategory) => {
    setMemories((prev) => prev.map((m) => m.id === id ? { ...m, key, value, category, updatedAt: Date.now() } : m));
    setEditingId(null);
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-3xl mx-auto">
      ...
      {/* Memory Providers */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-card border border-border overflow-hidden">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-3">
          {MEMORY_PROVIDERS.map((p) => (
            <button
              key={p.id}
              onClick={() => { setActiveProvider(p.id); if (!p.active) toast.info(`${p.label} — coming soon`); }}
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <p.icon className={`w-4 h-4 ${activeProvider === p.id ? "text-primary" : "text-muted-foreground"}`} />
                {activeProvider === p.id && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-primary/20 text-primary font-bold">ACTIVE</span>}
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {(["all", ...Object.keys(CATEGORY_META)] as const).map((cat) => (
            ...
            >
              {cat === "all" ? "ทั้งหมด" : `${CATEGORY_META[cat as MemoryCategory].emoji} ${CATEGORY_META[cat as MemoryCategory].label}`}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Memory list */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-2xl bg-card border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center gap-2">
          <Tag className="w-4 h-4 text-primary" />
          <span className="font-bold text-sm text-foreground">Stored Memories</span>
          <span className="ml-auto text-xs text-muted-foreground">{filtered.length} / {memories.length}</span>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="px-4 py-8 text-center text-sm text-muted-foreground italic">
            {searchQuery ? "ไม่พบ memory ที่ค้นหา" : "ยังไม่มี memory — กด 'เพิ่ม Memory' เพื่อเริ่ม"}
          </div>
        ) : (
                ...
                >
                  {editingId === entry.id ? (
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 group hover:bg-muted/20 transition-colors">
                      <span className="text-base shrink-0">{CATEGORY_META[entry.category].emoji}</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider truncate">{entry.key}</div>
                        <div className="text-sm text-foreground mt-0.5 truncate">{entry.value}</div>
                      </div>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold shrink-0 ${CATEGORY_META[entry.category].color}`}>
                        {CATEGORY_META[entry.category].label}
                      </span>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditingId(entry.id)} className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteMemory(entry.id)} className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
      ...
      {/* Recent commands */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-2xl bg-card border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center gap-2">
          <Clock className="w-4 h-4 text-primary" />
          <span className="font-bold text-sm text-foreground">คำสั่งล่าสุด</span>
        </div>
        {log.length === 0 ? (
          <div className="px-4 py-6 text-center text-sm text-muted-foreground italic">ยังไม่มีคำสั่ง</div>
        ) : (
          <div className="divide-y divide-border">
            {log.slice(0, 8).map((entry) => (
              <div key={entry.id} className="flex items-center justify-between px-4 py-2.5">
                <span className="text-sm text-foreground truncate flex-1 mr-3">{entry.command}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${
                  entry.status === "success" ? "bg-status-success/10 text-status-success" : "bg-destructive/10 text-destructive"
                }`}>
      ...
      {/* Danger zone */}
      {memories.length > 0 && (
        <button onClick={clearAll} className="flex items-center gap-2 text-xs text-destructive hover:text-destructive/80 transition-colors">
          <Trash2 className="w-3.5 h-3.5" />
          ล้าง Memory ทั้งหมด
        </button>
      )}
