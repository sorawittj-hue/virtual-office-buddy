...
import { hermesProxyApi, type HermesSession } from "@/lib/hermes-proxy-api";
...
export function SessionsPage() {
  ...
  useEffect(() => {
    ...
    hermesProxyApi
      .sessions()
      .then((sessions) => {
        if (!cancelled) setHermesSessions(sessions);
      })
      .catch(() => {
        if (!cancelled) setHermesSessions([]);
      })
      .finally(() => {
        if (!cancelled) setLoadingHermes(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  if (connectionMode === "hermes") {
    const filteredHermes = hermesSessions.filter((session) => {
      const name = session.name ?? session.title ?? session.id;
    ...
    return (
      <div className="p-6 lg:p-8 space-y-6 max-w-3xl mx-auto">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Real sessions from Hermes Agent ({hermesSessions.length} items)
            </p>
          </div>
        ...
        {loadingHermes ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <Clock className="w-12 h-12 text-muted-foreground/30 mb-3" />
            <p className="text-muted-foreground text-sm">No Hermes sessions found</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filteredHermes.map((session) => {
              const name = session.name ?? session.title ?? `Session ${session.id}`;
              const messageCount =
                session.message_count ?? session.messageCount ?? session.messages?.length ?? 0;
              const lastActive =
                ...
                <div key={session.id} className="rounded-xl border border-border bg-card p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Link
                      to="/chat"
                      onClick={() =>
                        localStorage.setItem("prism-active-hermes-session", session.id)
                      }
